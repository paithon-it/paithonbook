...
da predire $y$ vanno distinte — con parametri $\theta$: bassa dove i dati
sono plausibili, alta altrove. L'inferenza è un'ottimizzazione,
...
Cinque tappe, in salita dolce. Si comincia da dove l'idea è nata: la
**memoria associativa** di Hopfield {cite}`hopfield1982neural`, venticinque
neuroni che ricostruiscono un ricordo rovinato rotolando in fondo a una valle
— con il codice per vederlo accadere. Poi la **macchina di Boltzmann**
...
strade che l'hanno aggirato — campionare (MCMC e dinamica di Langevin),
stimare senza normalizzare (*score matching* {cite}`hyvarinen2005estimation`,
la sua forma denoising {cite}`vincent2011connection`, la stima contrastiva
col rumore {cite}`gutmann2010noise`) — e mostra come la seconda sia finita, un
decennio dopo, dentro i modelli di diffusione.
