...
del foglio c'è un rettangolino nero, minuscolo, con dentro due parole:
*codice ML*. È l'unico pezzo di cui parlano di solito i libri, i corsi, i
paper: il modello, l'algoritmo, la rete che impara. Tutto intorno, a
soffocarlo, ci sono scatole molto più grandi — raccolta dati, verifica dei
dati, estrazione delle *feature*, gestione della configurazione,
infrastruttura di *serving*, monitoraggio, strumenti di analisi, gestione
delle risorse di calcolo. La morale della figura è brutale e onesta:
...
Il modello addestrato è il piatto: la ricetta funziona. MLOps è tutto il
resto del ristorante. E come in un ristorante, il grosso dei guai non viene
dalla ricetta — viene dal magazzino che si svuota, dal fornitore che cambia
gli ingredienti, dalla cucina che va tenuta pulita ogni giorno.
...
per i modelli, pipeline automatizzate che rieseguono l'intero percorso — da dato
grezzo a modello servito — con un comando solo.
...
capitolo di Machine Learning, dove lo abbiamo inquadrato in termini statistici.
Quando succede, si torna all'inizio: nuovi dati, nuovo addestramento. Il
percorso non è una linea, è un **anello** ({numref}`fig-mlops-ciclo-vita`).
...
Un modello non è un quadro che appendi al muro e dimentichi: è un giardino.
Lo pianti (i dati), lo fai crescere (l'addestramento), controlli che sia sano
(la valutazione), lo apri al pubblico (il deploy) — e poi devi *tornarci ogni
giorno*: annaffiare, potare, accorgerti se una pianta si ammala
...
addestramento, valutazione, deploy — sono quelle che già conosci. La quinta,
il monitoraggio, è quella nuova: è l'occhio che sta in produzione e, quando
vede che i dati sono cambiati, tira la freccia che riporta all'inizio.
...
necessità di **riaddestrare** come operazione ordinaria, non come eccezione.
È questa la ragione strutturale per cui il ciclo è un anello e non un segmento.
...
- **Dal notebook alla produzione** — che cosa cambia quando si esce
  ...
  artefatti (dati, codice, modello), esperimenti tracciabili, il debito tecnico
  da tenere a bada.
...
- **Deployment e serving** — come si mette un modello *in ascolto*: servizio in
  tempo reale contro elaborazione in *batch*, latenza e *throughput*,
  rilasci graduali (*canary*, *shadow*, A/B) per non rompere niente.
