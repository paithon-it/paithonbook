...
- {doc}`GraphSAGE, GAT e applicazioni </GraphNeuralNetwork/architetture-applicazioni>` (Graph Neural Network). I Graph Transformer, cioè cosa succede a lasciar parlare ogni nodo con ogni altro invece che solo con i vicini. Se ne ricava una sorpresa che lega due capitoli lontani: la firma numerica con cui i Transformer segnano la posizione delle parole in una frase è lo stesso oggetto che qui segna la posizione di un nodo in un grafo, applicato al grafo più semplice che esista, una fila. Ed è verificato nella pagina.
