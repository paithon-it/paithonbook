...
**locale**: riguarda una predizione sola, la risposta del modello su quel caso
lì (che in formule si scrive $f(x_0)$: il modello $f$ applicato a quel caso
$x_0$), non l'intero comportamento del modello. Qui vediamo tre modi di
risponderle, tre attrezzi diventati lo
standard di fatto: **LIME**, **SHAP** e le spiegazioni **controfattuali**.
...
**attribuzioni** $\phi_j$ specifiche di $x_0$, una per feature, che dicono
quanto ciascuna ha spinto *questa* predizione sopra o sotto un riferimento.
...
così: prende la pratica di Maria e ne genera tante **varianti** leggermente
modificate (un po' più di reddito, un debito in meno, un'età diversa) e per
ciascuna chiede alla scatola nera cosa risponderebbe. Ottiene così una nuvola
di esempi-fantasma con le relative risposte del modello, tutti raccolti
*intorno* al caso di Maria.
...
capitolo di machine learning. Con una regola in più: i fantasmi più simili alla
pratica vera di Maria contano di più, quelli lontani quasi niente, ed è tutto
qui il senso della parola «pesata». I coefficienti di quella retta *sono* la
spiegazione: «il
reddito basso ha spinto verso il no di tanto, i debiti di tanto, l'anzianità
di lavoro ha spinto un po' verso il sì». È come tracciare la tangente a una
curva: non descrive tutta la strada, ma dice benissimo la pendenza nel punto
in cui ti trovi.
...
esempio il numero di feature non nulle, per una spiegazione corta).
...
In pratica si campionano dei punti $z$, si valuta $f(z)$ (la sola
...
appresi sono le attribuzioni. Conviene guardare da vicino **dove entra la
località**, perché non è dove ci si aspetta: nell'implementazione tabellare
degli autori i campioni non si generano attorno a $x_0$, ma dalla distribuzione
marginale del training set (l'opzione `sample_around_instance` è falsa per
default, e le colonne continue vengono per giunta discretizzate in quartili
prima di essere perturbate). La vicinanza a $x_0$ la introduce **solo** il peso
$\pi_{x_0}$, a valle. La conseguenza è concreta: la fedeltà locale dipende da
quanti dei punti generati globalmente cadono davvero vicino a $x_0$, e in alta
dimensione sono pochi.
...
esattamente le cose ovvie che abbiamo appena usato, e conviene chiamarle per
nome perché il resto del capitolo le nomina. Che il conto torni senza avanzi si
...
{cite}`sundararajan2020many`, e gli assiomi garantiscono l'unicità solo *a $v$
fissata*. Non è un difetto della formula, è di nuovo la forcella dell'apertura
...
colonna può esserci o non esserci, quindi con 30 colonne i gruppi possibili
sono due moltiplicato per sé stesso trenta volte, oltre un miliardo. Nel 2017
Scott Lundberg e Su-In Lee
...
casuali, gradient boosting, i protagonisti del capitolo sugli alberi ed
ensemble), usa **TreeSHAP**, che sfrutta la struttura ad albero per calcolare
i valori **esatti** in tempo ragionevole, senza campionare nulla.
...
diminuire (una monotonia che l'importanza da impurità della prima sezione
...
parte da sinistra dal **valore base**, cioè la risposta media del modello
quando non sa niente di questo cliente (scritta $\mathbb{E}[f(x)]$, qui
$0{,}20$), e si impilano i contributi, uno per riga: le barre che vanno verso
destra, in terracotta, alzano la predizione; quella che torna verso sinistra,
in teal, la abbassa. La seconda linea verticale è la risposta per questo
cliente, $0{,}44$, e la somma dei contributi ci arriva esattamente: è la
...
Wachter e colleghi cercano un punto $x_{\mathrm{cf}}$ che ottenga l'esito
desiderato $y'$ restando il più vicino possibile all'istanza originale $x_0$,
minimizzando

$$
\operatorname*{arg\,min}_{x_{\mathrm{cf}}}\;
   \lambda\,\big(f(x_{\mathrm{cf}}) - y'\big)^2
   + d(x_{\mathrm{cf}}, x_0),
$$

dove il primo termine spinge la predizione verso il valore-bersaglio $y'$ (la
soglia di approvazione) e $d$ misura quanto $x_{\mathrm{cf}}$ si discosta da
$x_0$: nel
...
perturbazione minima di $x_0$ che cambia l'uscita del modello è, formalmente,
...
$\mathcal{P}$ condizionata al fatto che $A$ resti soddisfatto, il modello
mantenga la stessa predizione con alta probabilità:

$$
\operatorname{prec}(A) = \mathbb{E}_{z \sim \mathcal{P}(\cdot \mid A)}
...
$\operatorname{cov}(A) = \mathbb{E}_{z\sim\mathcal{P}}[\mathbb{1}[A(z)]]$. La
...
valore vero mette quello di un riferimento fisso $r$, qui tutti zeri. Il valore
base è quindi $f(r)$, cioè la risposta del modello quando è spento tutto, e la
proprietà da verificare, l'**efficienza**, diventa
$\sum_i \phi_i = f(x) - f(r)$: la somma dei tre meriti deve fare esattamente la
differenza fra la risposta vera e quella base.
...
sola. È il caso su cui vale la pena vedere Shapley al lavoro. Le variabili si
numerano da zero, come si usa in Python: la «feature 0» è la prima, la «feature
1» la seconda, la «feature 2» la terza.
...
E la somma $1{,}5 + 2{,}0 + 0{,}5 = 4{,}0$ coincide con
$f(x) - f(\text{base})$:
...
- **LIME** costruisce, attorno al caso da spiegare, tanti casi-fantasma
  leggermente diversi, chiede al modello che cosa risponderebbe per ciascuno, e
  a quella nuvola adatta una retta contando di più i fantasmi più simili al
  caso vero. I pesi di quella retta sono la spiegazione. Funziona con qualunque
  modello, ma è **instabile**: rilanciato sullo stesso caso dà numeri diversi,
  e cambia anche a seconda di quanto largo si prende il vicinato e di come si
  è deciso di spezzettare l'input in parti.
...
- **LIME** {cite}`ribeiro2016why` approssima il modello con un **surrogato
  ...
  prossimità: la località la porta il peso $\pi_{x_0}$, non il campionamento. È
