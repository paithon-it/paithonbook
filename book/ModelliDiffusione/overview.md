...
finché non resta che neve televisiva: questo è il verso facile, la goccia che
si disperde. Poi addestra una rete a percorrere la pellicola all'indietro, un
fotogramma per volta. Se impara bene, potrà partire da neve pura (neve nuova,
...
mestiere di un restauratore paziente, che non ridipinge il quadro ma solleva
una velatura di sporco alla volta. Ed è un compito facile da imparare, perché
...
schermata di neve fresca e si applica la pulitura mille volte, dal passo 1.000
al passo 1. A ogni passo emerge qualcosa (una massa scura, una sagoma, un
gatto) e all'ultimo fotogramma c'è un'immagine che non esisteva da nessuna
parte: la rete ha imparato che aspetto ha il mondo, e la neve iniziale, sempre
diversa, decide quale immagine del mondo verrà fuori.
...
2022. Nel giro di pochi mesi OpenAI presenta DALL·E 2 (aprile), Google
      ...
      rilasciato con pesi aperti, scaricabili da chiunque. Per la prima volta
...
cambiale, e vale la pena dire subito *perché* il testimone è passato, e che
cosa la diffusione paga in cambio.
...
alcuni modi della distribuzione; il *mode collapse*, patologia strutturale del
gioco avversario, qui non ha un meccanismo per manifestarsi. Dhariwal e Nichol
{cite}`dhariwal2021diffusion` misurano infatti sia miglior FID sia miglior
copertura (*recall*) rispetto alle GAN di riferimento su ImageNet.
...
inverso di DDPM {cite}`ho2020denoising`, la derivazione della loss, la U-Net
che fa da spina dorsale e il ciclo di addestramento in PyTorch. Poi il **salto
di scala**: l'idea dei *latent diffusion models* {cite}`rombach2022high` di
spostare la diffusione in uno spazio latente compresso (il segreto che rende
possibile Stable Diffusion su una GPU da videogiochi) e il condizionamento sul
testo che trasforma un generatore di immagini in un illustratore su richiesta.
Infine **l'incontro con i Transformer** {cite}`vaswani2017attention`:
l'architettura DiT di Peebles e Xie {cite}`peebles2023scalable` che
sostituisce la U-Net con un Transformer su patch (la stessa ricetta che,
